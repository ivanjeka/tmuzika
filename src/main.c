...
static void glava(WINDOW *win, FileManager *fm);
...
void pusti_izabranu_pesmu(){
	...
	if (radio_bin && radio.online){
		...
		gst_object_unref(radio_bin);
		radio_bin = NULL;
		radio.online = 0;
		radio.svira = -1;
	}
	gst_element_set_state(reprodukcioni_blok, GST_STATE_NULL);
	gst_element_get_state(reprodukcioni_blok, NULL, NULL, 0);
...
void sledeca_nasumicna_pesma(){
	if (!nasumicno_red ||
	    nasumicno_velicina != playlist.stavke->len ||
	    nasumicno_pozicija >= nasumicno_velicina){
		napravi_nasumican_red();
	}

	if (!nasumicno_red || nasumicno_velicina == 0){
		return;
	}
...
void footer_lista(WINDOW *win){
	...
	const char *tekst; 
	if (radio_prozor){
		tekst = _(" ENTER/SPACE:play/pause ↑↓:move m:manager s:search BACKSPACE:back ");
	} else{
		tekst = _(" ENTER/SPACE:play/pause ↑↓:move m:manager s:search ");
	}
	...
	// FOOTER TEKST
	if (radio_prozor){
		wattron(win, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_TEKST_PAIR));
		mvwprintw(win, fy, 2, "%s", tekst);
		wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_TEKST_PAIR));
...
void footer_pomoc(WINDOW *win){
	...
	wattron(win, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_TEKST_PAIR));
	mvwprintw(win, fy, 2, _(" ↑↓:scroll_bottom_panel PgUp/PgDn:scroll_top_panel BACKSPACE:back "));
	wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_TEKST_PAIR));
}
void footer_opcije(WINDOW *win){
	int h, w;
	getmaxyx(win, h, w);
	int fy = h - 1;
	mvwprintw(win, fy, 0, "%*s", w, "");
	const char *tekst = _(" ENTER:open b:reset_colors BACKSPACE:back ");
	wchar_t wtekst[256];
	mbstowcs(wtekst, tekst, 256);
	int len = wcswidth(wtekst, wcslen(wtekst));
	int start_linije = 2 + len;
	if (start_linije >= w - 1){
		start_linije = w - 2;
	...
	int duzina_linije = w - 1 - start_linije;
	wattron(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
	mvwaddch(win, fy, 0, ACS_LLCORNER);
	mvwaddch(win, fy, 1, ACS_HLINE);
	wattroff(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
	wattron(win, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_TEKST_PAIR));
	mvwprintw(win, fy, 2, "%s", tekst);
	wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_TEKST_PAIR));
	...
	wattron(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
	mvwaddch(win, fy, w - 1, ACS_LRCORNER);
	wattroff(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
	wrefresh(win);
}
void footer_poruka(WINDOW *win){
	int h, w;
	getmaxyx(win, h, w);
	int fy = h - 1;
	mvwprintw(win, fy, 0, "%*s", w, ""); 
	...
	mvwprintw(win, fy, 0, "└─");
	wattroff(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
	wattron(win, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_OBAVESTENJE_PAIR));
	mvwprintw(win, fy, 2, "%s", footer_text);
	wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_OBAVESTENJE_PAIR));
	wattron(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
	if (duzina_linije > 0){
		mvwhline(win, fy, start_linije, ACS_HLINE, duzina_linije);
	}
...
void footer_datoteka(WINDOW *win){
	int h, w;
	getmaxyx(win, h, w);
	int fy = h - 1;
	mvwprintw(win, fy, 0, "%*s", w, "");
	const char *tekst = _(" ENTER:open/add ESC:parent f:add_folder o:options BACKSPACE:back ");
	wchar_t wtekst[256];
	mbstowcs(wtekst, tekst, sizeof(wtekst)/sizeof(wchar_t));
	int len = wcswidth(wtekst, wcslen(wtekst));
	int start_linije = 2 + len;
...
static gint poredjaj_fajlove_abecedno(gconstpointer a, gconstpointer b){
	...
	// ARHIVA
	const char *archive_exts[] ={ ".zip", ".rar", ".7z", ".tar", ".gz", ".bz2", ".xz", ".tgz", ".tar.gz", ".tar.bz2", ".tar.xz", ".cab", ".iso", ".nrg", NULL };
...
static void fm_crtaj(WINDOW *win, FileManager *fm){
	...
	int h, w;
	getmaxyx(win, h, w);
	/*wattron(win, COLOR_PAIR(TMUZIKA_PANEL_TMUZIKA_BG));
	for (int r = 1; r <= 6; r++) {
		mvwhline(win, r, 1, ' ', w - 2);
	}
	wattroff(win, COLOR_PAIR(TMUZIKA_PANEL_TMUZIKA_BG));
	wattron(win, COLOR_PAIR(TMUZIKA_NASLOV_PAIR) | A_BOLD);
	mvwprintw(win, 3, 2, _("tmuzika - terminal music player"));
	wattroff(win, COLOR_PAIR(TMUZIKA_NASLOV_PAIR) | A_BOLD);
	wattron(win, COLOR_PAIR(TMUZIKA_POMOC_PAIR));
	mvwprintw(win, 4, 2, _("? = help  o = options  k = exit app"));
	wattroff(win, COLOR_PAIR(TMUZIKA_POMOC_PAIR));
	wattron(win, COLOR_PAIR(TMUZIKA_POMOC_PAIR));
	mvwprintw(win, 5, 2, _("File Manager"));
	wattroff(win, COLOR_PAIR(TMUZIKA_POMOC_PAIR));
	wattron(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
	mvwhline(win, 7, 1, ACS_HLINE, w - 2);
	wattroff(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
	int path_max_w = w - 4;
	wattron(win, COLOR_PAIR(TMUZIKA_PANEL_FOLDER_BG));
	mvwhline(win, 8, 1, ' ', w - 2);
	wattroff(win, COLOR_PAIR(TMUZIKA_PANEL_FOLDER_BG));
	wattron(win, COLOR_PAIR(TMUZIKA_TEKST_NASLOV_PAIR));
	mvwprintw(win, 8, 2, "%.*s", path_max_w, fm->trenutni_put);
	wattroff(win, COLOR_PAIR(TMUZIKA_TEKST_NASLOV_PAIR));
	wattron(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
	mvwhline(win, 9, 1, ACS_HLINE, w - 2);
	wattroff(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));*/
	glava(win, fm);
	int list_start = 10;
	mvwhline(win, h - 2, 1, ' ', w - 2);
	int footer_height = 2; // linija + tekst
	int max_prikaz = h - list_start - footer_height;
	for (int i = 0; i < max_prikaz; i++){
		...
		if (idx == fm->selektovan){
			wattron(win, COLOR_PAIR(TMUZIKA_PLAYLIST_MARK_PAIR) | A_BOLD);
			mvwprintw(win, y, 1, ">");
			wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLIST_MARK_PAIR) | A_BOLD);
		} else{
			mvwprintw(win, y, 1, " ");
		}
		// prefiks i boja
		const char *prefix;
		int pair = TMUZIKA_OSTALO_PAIR;
		if (is_dir){
			prefix = "[DIR] ";
			pair = TMUZIKA_DIREKTORIJUM_PAIR;
		} else {
			const char *ext = strrchr(ime, '.');
			if (ext){
				if (g_ascii_strcasecmp(ext, ".m3u") == 0){
					pair = TMUZIKA_AUDIO_PAIR;
					prefix = "[M3U] ";
				} else if (g_ascii_strcasecmp(ext, ".mp3") == 0 ||
		...
		if (idx == fm->selektovan){
			...
			wattron(win, COLOR_PAIR(TMUZIKA_FM_SELECTEDFILE_PAIR) | A_BOLD);
		} else {
			wattron(win, COLOR_PAIR(pair));
		}
		// horizontalni scroll celog reda (prefiks + ime)
		char line[PATH_MAX + 64];
		snprintf(line, sizeof(line), "%s%s", prefix, ime);
		int col_start = 3;
		int avail_w = w - col_start - 1;
		int line_len = strlen(line);
		int offset = fm->x_offset;
		if (offset > line_len){
			offset = line_len;
		}
		int prikaz_len = line_len - offset;
		if (prikaz_len > avail_w){
			prikaz_len = avail_w;
		}
		mvwprintw(win, y, col_start, "%.*s", prikaz_len, line + offset);
		// isključi boju
		if (idx == fm->selektovan){
			wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLIST_MARK_PAIR) | A_BOLD);
		} else if (marked){
			wattroff(win, COLOR_PAIR(TMUZIKA_FM_SELECTEDFILE_PAIR) | A_BOLD);
		} else {
	...
	wrefresh(win);
	if (fm_cpu_ciscenje) fm_cpu_ciscenje = FALSE;
}
int popup_lista(WINDOW *win, const char **items){
	...
	while (1) {
		...
		box(win, 0, 0);
		wattroff(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
		wattron(win, COLOR_PAIR(TMUZIKA_KONFIGURACIJA_NASLOV_PAIR));
		mvwprintw(win, 1, 2, _("Configuration files"));
		wattroff(win, COLOR_PAIR(TMUZIKA_KONFIGURACIJA_NASLOV_PAIR));
		wattron(win, A_DIM);
		mvwprintw(win, 2, 2, _("Select a config file to open in editor"));
		wattroff(win, A_DIM);
		for (int i = 0; items[i]; i++) {
			if (i == izbor){
				wattron(win, COLOR_PAIR(TMUZIKA_PLAYLISTMARK_PAIR) | A_REVERSE);
			}
			mvwprintw(win, i+4, 2, "%s", items[i]);
			if (i == izbor){
				wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLISTMARK_PAIR) | A_REVERSE);
			}
		}
		footer_opcije(win);
		wrefresh(win);
		...
		switch (ch) {
			...
			/*case 'h':{
				tmuzika_reset_komande_prompt();
				break;
			}*/
			case 'b':{
				tmuzika_reset_boje_prompt();
				break;
			}
		}
	}
}
static void fm_otvori_config(const char *ime_fajla){
	...
	snprintf(putanja, sizeof(putanja), "%s/.config/tmuzika/%s", g_get_home_dir(), ime_fajla);
	endwin();
	pid_t pid = fork();
	if (pid == 0){
		execlp(editor, editor, putanja, (char *)NULL);
		_exit(127);
	} else if (pid > 0){
		waitpid(pid, NULL, 0);
		// TMUZIKA
...
static int fm_polje_za_pretragu(char *buf, size_t size){
	WINDOW *w = newwin(3, COLS - 4, LINES - 5, 2);
	keypad(w, TRUE);
	size_t len = 0;
	buf[0] = '\0';
	curs_set(1);
	while (1){
		werase(w);
		wattron(w, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
		box(w, 0, 0);
		wattroff(w, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
		int maxx = getmaxx(w);
		const char *label = _(" Search file or folder: ");
		wchar_t wlabel[256];
		mbstowcs(wlabel, label, sizeof(wlabel) / sizeof(wchar_t));
		int label_len = wcswidth(wlabel, wcslen(wlabel));
		int visible = maxx - label_len - 4;
		if (visible < 1){
			visible = 1;
		}
		size_t offset = 0;
		...
		mvwprintw(w, 1, 2, "%s%s", label, buf + offset);
		wchar_t wbuf[512];
		mbstowcs(wbuf, buf + offset, sizeof(wbuf) / sizeof(wchar_t));
		int cursor_width = wcswidth(wbuf, wcslen(wbuf));
		wmove(w, 1, 2 + label_len + cursor_width);
		wrefresh(w);
		int ch = wgetch(w);
		if (ch == 27){
			buf[0] = '\0';
...
static int fm_key(FileManager *fm, int ch){
	...
	switch (ch){
		...
		case 10:{ // ENTER
			...
			if (stat(novi_put, &st) == 0 && S_ISDIR(st.st_mode)){
				...
				if (ext && g_ascii_strcasecmp(ext, ".m3u") == 0){
					...
					footer_prikaz_poruka(stdscr, _(" Unsupported file type "));
				}
			}
			g_free(novi_put);
			break;
		...
		case 14:{ // napravi novu fasciklu u popup-u
			WINDOW *win = stdscr;
			char ime_nove_fascikle[256];
			int unos = zahtev_unosa_esc(win, _("Enter new folder name: "), ime_nove_fascikle, sizeof(ime_nove_fascikle));
			if (!unos || ime_nove_fascikle[0] == '\0'){
				fm_cpu_ciscenje = TRUE;
				break;
			}
			char *puni_put = g_build_filename(fm->trenutni_put, ime_nove_fascikle, NULL);
			if (g_file_test(puni_put, G_FILE_TEST_IS_DIR)){
				footer_prikaz_poruka(stdscr, _(" Folder \"%s\" already exists "), ime_nove_fascikle);
				g_free(puni_put);
				break;
			}
		...
		case 20:{ // napravi novi dokument
			WINDOW *win = stdscr;
			char ime_nove_datoteke[256];
			int unos = zahtev_unosa_esc(win, _("Enter new text file name: "), ime_nove_datoteke, sizeof(ime_nove_datoteke));
			if (!unos || ime_nove_datoteke[0] == '\0'){
				fm_cpu_ciscenje = TRUE;
				break;
			}
			char *puni_put = g_build_filename(fm->trenutni_put, ime_nove_datoteke, NULL);
			if (g_file_test(puni_put, G_FILE_TEST_EXISTS)){
				footer_prikaz_poruka(stdscr, _(" File \"%s\" exists. Overwrite? (y/N) "), ime_nove_datoteke);
				int ch = getch();
				if (ch != 'y' && ch != 'Y'){
					footer_prikaz_poruka(stdscr, _(" Creation canceled "));
					g_free(puni_put);
					fm_cpu_ciscenje = TRUE;
					break;
				}
		...
		case KEY_DC:{ //delete
			...
			for (guint t = 0; t < targets->len; t++){
				...
				if (!multi){
					...
					char timestamp[32];
					sprintf(timestamp, "_%04d%02d%02d%02d%02d%02d", tm.tm_year+1900, tm.tm_mon+1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
					g_free(fm->undo_path_src);
					g_free(fm->undo_path_dest);
					fm->undo_path_src = g_build_filename("/tmp/fm_undo", g_strdup_printf("%s%s", ime, timestamp), NULL);
					fm->undo_path_dest = g_strdup(puni_put);
		...
		case 's':{
			char query[256];
			if (!fm_polje_za_pretragu(query, sizeof(query))){
				fm_cpu_ciscenje = TRUE;
				break;
			}
			if (query[0] == '\0'){
		...
		case 'v':{ // v - zalepi
			...
			for (guint i = 0; i < fm->clipboard->len; i++){
				...
				if (g_file_test(dest, G_FILE_TEST_EXISTS)){
					footer_prikaz_poruka(stdscr, _(" \"%s\" exists. Overwrite? (y/N) "), g_path_get_basename(source));
					int ch_overwrite = getch();
					if (ch_overwrite != 'y' && ch_overwrite != 'Y'){
						g_free(dest);
						continue;
					}
					if (g_file_test(dest, G_FILE_TEST_IS_DIR)){
						gchar *cmd_rm = g_strdup_printf("rm -rf \"%s\"", dest);
						system(cmd_rm);
						g_free(cmd_rm);
					} else {
						remove(dest);
				...
				if (!multi){
					g_free(fm->undo_path_src);
					g_free(fm->undo_path_dest);
					fm->undo_path_src = NULL;
					fm->undo_path_dest = NULL;
				...
				if (fm->clipboard_mode == CLIPBOARD_COPY){
					if (g_file_test(source, G_FILE_TEST_IS_DIR)){
						gchar *cmd_cp = g_strdup_printf("cp -r \"%s\" \"%s\"", source, dest);
						system(cmd_cp);
						g_free(cmd_cp);
					} else {
						gchar *cmd_cp = g_strdup_printf("cp \"%s\" \"%s\"", source, dest);
						system(cmd_cp);
						g_free(cmd_cp);
					}
				} else if (fm->clipboard_mode == CLIPBOARD_CUT){
					gchar *cmd_mv = g_strdup_printf("mv \"%s\" \"%s\"", source, dest);
					system(cmd_mv);
					g_free(cmd_mv);
				}
				g_free(dest);
			...
			if (multi){
				footer_prikaz_poruka(stdscr, _(" Pasted %d items (undo disabled) "), fm->clipboard->len);
			}
		...
		case KEY_F(2): { // F2 Rename
			WINDOW *win = stdscr;
			if (fm->stavke->len == 0){
				break;
			}
			char *old_name = g_ptr_array_index(fm->stavke, fm->selektovan);
			char *old_path = g_build_filename(fm->trenutni_put, old_name, NULL);
			char new_name[256];
			...
			char prikaz_old[128];
			g_snprintf(prikaz_old, sizeof(prikaz_old), "%.*s%s", max_prikaz, old_name, strlen(old_name) > (size_t)max_prikaz ? "..." : "");
			char prompt[512];
			snprintf(prompt, sizeof(prompt), _("Rename \"%s\" to: "), prikaz_old);
			int unos = zahtev_unosa_esc(win, prompt, new_name, sizeof(new_name));
			if (!unos || new_name[0] == '\0'){
				footer_prikaz_poruka(stdscr, _(" Rename canceled "));
				g_free(old_path);
				fm_cpu_ciscenje = TRUE;
				break;
			}
			char *new_path = g_build_filename(fm->trenutni_put, new_name, NULL);
			...
			if (rename(old_path, new_path) == 0){
				char prikaz_new[128];
				g_snprintf(prikaz_new, sizeof(prikaz_new), "%.*s%s", max_prikaz, new_name, strlen(new_name) > (size_t)max_prikaz ? "..." : "");
				footer_prikaz_poruka(stdscr, _(" Renamed \"%s\" → \"%s\" "), prikaz_old, prikaz_new);

			} else {
				footer_prikaz_poruka(stdscr, _(" Failed to rename \"%s\" "), prikaz_old);
			}
			g_free(old_path);
			g_free(new_path);
			fm_ucitaj(fm);
		...
		case ']': { // scroll desno
			WINDOW *win = stdscr;
			int visible = getmaxx(win) - 4;
			int max_len = 0;
			for (guint i = 0; i < fm->stavke->len; i++){
				char *ime = g_ptr_array_index(fm->stavke, i);
				char *puni_put = g_build_filename(fm->trenutni_put, ime, NULL);
				struct stat st;
				int is_dir = (stat(puni_put, &st) == 0 && S_ISDIR(st.st_mode));
				g_free(puni_put);
				int len;
				if (is_dir){
					len = strlen("[DIR] ") + strlen(ime);
				} else {
					len = strlen("[   ] ") + strlen(ime);
				}
				if (len > max_len){
					max_len = len;
				}
			}
			int remaining = max_len - fm->x_offset - visible;
			if (remaining > 0){
				int step = visible / 2;
				if (step < 1){
					step = 1;
		...
		case 'o':{
			const char *konfiguracione_stavke[] = {
				//"tmuzika_komande",
				"tmuzika_boje"
			};
			const char *prikaz[] = { 
			...
			while (1){ // petlja za popup
				int izbor = popup_lista(stdscr, prikaz);
				if (izbor < 0){
					break;
				}
				fm_otvori_config(konfiguracione_stavke[izbor]);
				//footer_prikaz_poruka(stdscr, _("Returned from editor, press ESC to go back"));
			}
			fm_cpu_ciscenje = TRUE;
			break;
		}
		...
		default:{
			...
			} else if (ch == komande.tmuzika_volume_up){
				if (radio_bin){
			...
			} else if (ch == komande.tmuzika_volume_down){
				if (radio_bin){
					jacina_zvuka_radio(radio.volume - JACINA_KORAK);
					radio_cpu_ciscenje = 1;
				} else {
			...
			} else if (ch == komande.tmuzika_stop){
				if (radio_bin){
					radio_zaustavi();
					radio_cpu_ciscenje = 1;
				} else {
			...
			} else if (ch == komande.tmuzika_pause){
				if (radio_bin){
					pauziraj_radio();
					radio_cpu_ciscenje = 1;
				} else {
			...
			} else if (ch == komande.tmuzika_next){
				if (radio_bin){
			...
			} else if (ch == komande.tmuzika_preview){
				if (radio_bin){
			...
			} else if (ch == komande.tmuzika_seek_f){
				ubrzaj_pesmu(SKOK_SEKUNDE);
				horizontal_scroll = 0;
				cpu_ciscenje = 1;
			} else if (ch == komande.tmuzika_seek_b){
				ubrzaj_pesmu(-SKOK_SEKUNDE);
				horizontal_scroll = 0;
				cpu_ciscenje = 1;
			}
...
void radio_prikaz(WINDOW *win){
	...
	int lista_pocetni_red = 10;
	int show = h - lista_pocetni_red - 2;
	if (show < 4){
		show = 4;
	}
	radio.visible_rows = show;
	// POZADINA
	wattron(win, COLOR_PAIR(TMUZIKA_PANEL_LISTA_BG));
	for (int r = lista_pocetni_red; r < h - 2; r++){
		mvwhline(win, r, 1, ' ', width);
	}
	wattroff(win, COLOR_PAIR(TMUZIKA_PANEL_LISTA_BG));
	// TRENUTNO SVIRA
	...
	if (radio.svira >= 0){
		...
		} else if (radio.stanice && radio.svira < (int)radio.stanice->len){
	...
	// PLAYLISTA
	int start = radio.trenutna - show / 2;
	if (start < 0){
		start = 0;
	}
	int row = lista_pocetni_red;
	for (int i = 0; i < show && (start + i) < (int)radio.stanice->len; i++){
		int idx = start + i;
		RadioStanica *rs = g_ptr_array_index(radio.stanice, idx);
		int svira_ova = 0;
		...
		if (svira_ova && radio.online){
			wattron(win, COLOR_PAIR(TMUZIKA_PLAYLIST_SVIRA_PAIR) | A_BOLD);
		} else if (idx == radio.trenutna){
			wattron(win, COLOR_PAIR(TMUZIKA_PLAYLISTMARK_PAIR) | A_REVERSE);
		} else {
			wattron(win, COLOR_PAIR(TMUZIKA_PLAYLIST_TEKST_PAIR));
		}
		char linija[512];
		snprintf(linija, sizeof(linija), "%s %2d: %s",
			play_mark,
			idx + 1,
			rs ? rs->name : ""
		);
		int maxw = w - 4;
		mvwprintw(win, row++, 2, "%-*s", maxw, linija);
		if (svira_ova && radio.online){
			wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLIST_SVIRA_PAIR) | A_BOLD);
		} else if (idx == radio.trenutna){
			wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLISTMARK_PAIR) | A_REVERSE);
		} else {
			wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLIST_TEKST_PAIR));
		}
	}
	// PRAZAN RED POZADINA
	if (row < h - 1){
	...
	// SCROLL
	if (radio.stanice->len > show){
		int scroll_h = show;
		int handle_h = scroll_h * show / radio.stanice->len;
		if (handle_h < 1){
			handle_h = 1;
		}
		int start_pos = (radio.trenutna * scroll_h) / radio.stanice->len;
		if (start_pos + handle_h > scroll_h){
			start_pos = scroll_h - handle_h;
		}
		int col_scroll = w - 2;
		wattron(win, COLOR_PAIR(TMUZIKA_SCROLL_PAIR));
		...
		wattroff(win, COLOR_PAIR(TMUZIKA_SCROLL_PAIR));
	}
	if (footer_privremeni){
		footer_poruka(win);
	} else {
...
void cuvanje_radio_m3u_liste(RadioLista *radio, const char *outpath){
	char path[MAX_DUZINA];
	strncpy(path, outpath, sizeof(path) - 1);
	path[sizeof(path) - 1] = '\0';
	skracena_putanja_foldera(path, sizeof(path));
	...
	fprintf(f, "#EXTM3U\n");
	for (guint i = 0; i < radio->stanice->len; ++i){
		RadioStanica *st = g_ptr_array_index(radio->stanice, i);
		if (st && st->url){
...
void glava(WINDOW *win, FileManager *fm){
	int width;
	...
	if(fm_prozor){
		...
		// POZADINA NASLOVA	
		int left_w = mid_col - 1;
		int path_max_w = width - 4;
		wattron(win, COLOR_PAIR(TMUZIKA_PANEL_TMUZIKA_BG));
		for (int r = 1; r <= 6; r++) {
			mvwhline(win, r, 1, ' ', left_w);
		}
		wattroff(win, COLOR_PAIR(TMUZIKA_PANEL_TMUZIKA_BG));
	
		// POPRECNA ILINIJA
		wattron(win, COLOR_PAIR(TMUZIKA_V_BORDER_PAIR));
		for (int r = 1; r <= 6; r++){
			mvwaddch(win, r, mid_col, ACS_VLINE);
		}
		wattroff(win, COLOR_PAIR(TMUZIKA_V_BORDER_PAIR));
		// BOJA NASLOVA
		wattron(win, COLOR_PAIR(TMUZIKA_NASLOV_PAIR) | A_BOLD);
		...
		// POZADINA PRIKAZA DESNO
		//int right_w = inner_w - mid_col - 1;
		wattron(win, COLOR_PAIR(TMUZIKA_PANEL_PRIKAZ_BG));
		for (int r = 1; r <= 6; r++) {
			mvwhline(win, r, mid_col + 1, ' ', getmaxx(win) - mid_col - 2);
		}
		wattroff(win, COLOR_PAIR(TMUZIKA_PANEL_PRIKAZ_BG));
		if (radio_bin){
			const char *status;
			status = radio.online ? (pauziranje ? _("LIVE: PAUSE") : _("LIVE: ONLINE")) : _("LIVE: OFFLINE");
			// BOJA JACINA
			wattron(win, COLOR_PAIR(TMUZIKA_VOL_PAIR));
			mvwprintw(win, 1, mid_col + 2, _("Volume: %.2f"), radio.volume);
			wattroff(win, COLOR_PAIR(TMUZIKA_VOL_PAIR));
			// BOJA STATUS
			wattron(win, COLOR_PAIR(TMUZIKA_STATUS_PAIR));
			mvwprintw(win, 2, mid_col + 2, "%s", status);
			wattroff(win, COLOR_PAIR(TMUZIKA_STATUS_PAIR));
			// BOJA VREME I DATUM
			...
			if (vreme_sada){
				...
				// BOJA VREME
				wattron(win, COLOR_PAIR(TMUZIKA_TIME_PAIR));
				mvwprintw(win, 3, mid_col + 2, _("Time: %s"), vreme_bufer);
				wattroff(win, COLOR_PAIR(TMUZIKA_TIME_PAIR));
				// BOJA DATUM
				wattron(win, COLOR_PAIR(TMUZIKA_DATE_PAIR));
				mvwprintw(win, 4, mid_col + 2, _("Date: %s"), datum_bufer);
				wattroff(win, COLOR_PAIR(TMUZIKA_DATE_PAIR));
			}
			// BOJA BROJA RADIO STANICA
			wattron(win, COLOR_PAIR(TMUZIKA_LIST_PAIR));
			mvwprintw(win, 5, mid_col + 2, _("Radio stations: %d"), (int)radio.stanice->len);
			wattroff(win, COLOR_PAIR(TMUZIKA_LIST_PAIR));
		} else {
			...
			wattroff(win, COLOR_PAIR(TMUZIKA_STATUS_PAIR));
			// BOJA PONAVLJANJA
			wattron(win, COLOR_PAIR(TMUZIKA_REPET_PAIR));
			...
			wattroff(win, COLOR_PAIR(TMUZIKA_REPET_PAIR));
			// BOJE VREME
		...
		wattroff(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
		// BOJA POZADINE DELA GDE PISE RADIO
		wattron(win, COLOR_PAIR(TMUZIKA_PANEL_FOLDER_BG)); // pozadina panela
		mvwhline(win, 8, 1, ' ', inner_w);      // popuni celu širinu
		wattroff(win, COLOR_PAIR(TMUZIKA_PANEL_FOLDER_BG));
		// BOJA TRADIO
		wattron(win, COLOR_PAIR(TMUZIKA_TEKST_NASLOV_PAIR));
		mvwprintw(win, 8, 2, "%.*s", path_max_w, fm->trenutni_put);
		...
		// BOJA TRADIO
		wattron(win, COLOR_PAIR(TMUZIKA_TEKST_NASLOV_PAIR));
		mvwprintw(win, 8, 2, _("tmuzika - RADIO"));
		wattroff(win, COLOR_PAIR(TMUZIKA_TEKST_NASLOV_PAIR));
		// BOJA OKVIRA
		wattron(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
		mvwhline(win, 9, 1, ACS_HLINE, inner_w);
		...
		// BOJA DELA GDE JE FOLDER
		//wattron(win, COLOR_PAIR(TMUZIKA_PANEL_FOLDER_BG) | A_BOLD);
		wattron(win, COLOR_PAIR(TMUZIKA_PANEL_FOLDER_BG));
		mvwhline(win, 8, 1, ' ', inner_w);
		char folder_txt[PATH_MAX + 32];
		wattron(win, COLOR_PAIR(TMUZIKA_TEKST_NASLOV_PAIR));
		snprintf(folder_txt, sizeof(folder_txt), _("Folder: %s"), kratka_putanja);
		print_fit(win, 8, 2, inner_w - 3, folder_txt);
		wattroff(win, COLOR_PAIR(TMUZIKA_TEKST_NASLOV_PAIR));
		//wattroff(win, COLOR_PAIR(TMUZIKA_PANEL_FOLDER_BG) | A_BOLD);
		wattroff(win, COLOR_PAIR(TMUZIKA_PANEL_FOLDER_BG));
		// BOJA OKVIR
		wattron(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
		mvwhline(win, 9, 1, ACS_HLINE, inner_w);
...
void osvezi_tmuzika_status() {
	...
	gint64 pos = 0, dur = 0;
	if (!gst_element_query_position(reprodukcioni_blok, fmt, &pos)){
		pos = 0;
	}
	if (!gst_element_query_duration(reprodukcioni_blok, fmt, &dur)){
		dur = 0;
	}
	cached_pos = pos;
	cached_dur = dur;
	GstState state;
	gst_element_get_state(reprodukcioni_blok, &state, NULL, GST_CLOCK_TIME_NONE);
	pustanje = (state == GST_STATE_PLAYING);
}
static void izvuci_artist_album_title(const char *uri, char *artist, size_t a_len, char *album, size_t al_len, char *title, size_t t_len){
...
static void prikaz_programa(WINDOW *win){
	...
	int bg_start = lista_pocetni_red;
	int bg_end = LINES - 2; // poslednja dva reda za footer
	wattron(win, COLOR_PAIR(TMUZIKA_PANEL_LISTA_BG));
	for (int r = bg_start; r < bg_end; r++){
		mvwhline(win, r, 1, ' ', width);
	}
	wattroff(win, COLOR_PAIR(TMUZIKA_PANEL_LISTA_BG));
	max_line_length = 0;
	...
	// BOJA PRAZNOG REDA PRE FOOTERA
	if (row < LINES - 1){ // da ne pređe footer
		wattron(win, COLOR_PAIR(TMUZIKA_PANEL_LISTA_BG));
		mvwhline(win, row, 1, ' ', width);
		wattroff(win, COLOR_PAIR(TMUZIKA_PANEL_LISTA_BG));
	}
...
static int zahtev_unosa_esc(WINDOW *win, const char *prompt, char *out, size_t outlen){
	...
	while (1){
		...
		int maxx = getmaxx(w);
		wchar_t wlabel[256];
		mbstowcs(wlabel, prompt, sizeof(wlabel) / sizeof(wchar_t));
		int label_len = wcswidth(wlabel, wcslen(wlabel));
		int visible = maxx - label_len - 4;
		if (visible < 1){
			visible = 1;
		...
		mvwprintw(w, 1, 2, "%s%s", prompt, out + offset);
		wchar_t wbuf[512];
		mbstowcs(wbuf, out + offset, sizeof(wbuf) / sizeof(wchar_t));
		int cursor_width = wcswidth(wbuf, wcslen(wbuf));
		wmove(w, 1, 2 + label_len + cursor_width);
		wrefresh(w);
		provera_zavrsetka_pesme();
		...
		} else if (isprint(ch) && len < outlen - 1){
			out[len++] = (char)ch;
			out[len] = '\0';
		}
	}
...
int main(int argc, char *argv[]){
	...
	if (has_colors()){
		...
		if (COLORS >= 16) {
			...
			cista_bela = COLOR_WHITE;
		}
		init_pair(CISTA_BELA, cista_bela, -1);
	...
	while (pokretanje){
		...
		if (radio_prozor){
			...
			if (k == ERR){
				usleep(10 * 1000);
				continue;
			} else if (k == komande.tmuzika_exit){
				werase(win);
				wattron(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
				box(win, 0, 0);
				wattroff(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
				...
				radio_cpu_ciscenje = 1;
			} else if (k == komande.tradio_volume_up){
				jacina_zvuka_radio(radio.volume + JACINA_KORAK);
			} else if (k == komande.tradio_volume_down){
				jacina_zvuka_radio(radio.volume - JACINA_KORAK);
        		} else if (k == komande.tradio_play || k == '\n'){
			...
			} else if (k == komande.tradio_stop){
				radio_zaustavi();
				radio_cpu_ciscenje = 1;
			} else if (k == komande.tradio_add){
				radio_dodaj(win);
				radio_cpu_ciscenje = 1;
			} else if (k == komande.tradio_delete_song){
				char potvrda[8] ={0};
				if (!radio.stanice || radio.stanice->len == 0){
					radio_cpu_ciscenje = 1;
					break;
			...
			} else if (k == komande.tradio_delete_all){
				char potvrda[8] ={0};
				if (!radio.stanice || radio.stanice->len == 0){
					radio_cpu_ciscenje = 1;
					break;
				...
				pomoc_komande(win);
				radio_cpu_ciscenje = 1;
			} else if (k == komande.tradio_file_manager){
				fm_prozor = 1;
				...
				g_ptr_array_free(fm.stavke, TRUE);
				radio_cpu_ciscenje = 1;
    			} else if (k == komande.tradio_pause){
				pauziraj_radio();
				radio_cpu_ciscenje = 1;
			} else if (k == komande.tradio_search){
				char query[256];
				backup_trenutna = radio.trenutna;
				while (1){
			...
			} else if (k == 27 && radio.backup_stanice){
				g_ptr_array_free(radio.stanice, TRUE);
				radio.stanice = radio.backup_stanice;
				radio.backup_stanice = NULL;
				if (backup_trenutna >= 0 && backup_trenutna < (int)radio.stanice->len){
					radio.trenutna = backup_trenutna;
				} else {
					radio.trenutna = radio.svira;
				}
				if (radio.trenutna < radio.visible_rows){
				...
				radio_cpu_ciscenje = 1;
			} else if (k == komande.tradio_current_song){
				if (radio.svira >= 0 && radio.svira < (int)radio.stanice->len){
			...
			} else if (k == komande.tradio_load_m3u){			
				char path[MAX_DUZINA];
				while (1){
			...
			} else if (k == komande.tradio_save_m3u){			
				char path[MAX_DUZINA];
				while (1){
					...
					if (unos && path[0]){
						cuvanje_radio_m3u_liste(&radio, path);
			...
			} else if (k == komande.tradio_next){
				pusti_sledecu_stanicu();
			} else if (k == komande.tradio_preview){
				pusti_prethodnu_stanicu();
			} else if (k == komande.tradio_first_song){
				pusti_prvu_stanicu();
			} else if (k == komande.tradio_last_song){
		...
		if (ch == ERR){
			usleep(10 * 1000);
			continue;
		} else if (ch == komande.tmuzika_radio){
			pustanje = 0;
			radio_prozor = 1;
			radio_cpu_ciscenje = 1;
		} else if (ch == komande.tmuzika_exit){
			werase(win);
			wattron(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
			box(win, 0, 0);
			wattroff(win, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
		...
		} else if (ch == komande.tmuzika_play || ch == '\n'){
			const char *uri = NULL;
			...
			horizontal_scroll = 0;
			cpu_ciscenje = 1;
		} else if (ch == komande.tmuzika_file_manager){
fm_prozor = 1;
			trenutni_rezim = REZIM_FILE_MANAGER;
			FileManager fm ={0};
			getcwd(fm.trenutni_put, sizeof(fm.trenutni_put));
			...
			g_ptr_array_free(fm.stavke, TRUE);
			cpu_ciscenje = 1;
		} else if (ch == komande.tmuzika_save_m3u){			
			char path[MAX_DUZINA];
			while (1){
				int unos = zahtev_unosa_esc(win, _("Save (.m3u) playlist: "), path, sizeof(path));
				if (!unos){
		...
		} else if (ch == komande.tmuzika_load_m3u){			
			char path[MAX_DUZINA];
			while (1){
			...
			pomoc_komande(win);
			cpu_ciscenje = 1;
		} else if (ch == komande.tmuzika_search){
			char query[256];
			while (1){
				provera_zavrsetka_pesme();
				...
				if (!unos || query[0] == '\0'){
					if (playlist.backup_stavke){
						...
						if (backup_trenutna < (int)playlist.stavke->len){
							playlist.trenutna = backup_trenutna;
						} else{
							playlist.trenutna = 0;
						}
					}
				...
				if (!playlist.backup_stavke){
					backup_trenutna = playlist.trenutna;
		...
		} else if (ch == 27 && playlist.backup_stavke){
			g_ptr_array_free(playlist.stavke, TRUE);
			playlist.stavke = playlist.backup_stavke;
			playlist.backup_stavke = NULL;
			if (backup_trenutna < (int)playlist.stavke->len){
			...
			cpu_ciscenje = 1;				
		} else if (ch == komande.tmuzika_delete_all){
			char potvrda[8] ={0};
			while (1){
		...
		} else if (ch == komande.tmuzika_delete_song){
			char potvrda[8] ={0};
			...
			if (f_ime) g_free(f_ime);
		} else if (ch == komande.tmuzika_current_song){
			if (playlist.svira >= 0 && playlist.svira < (int)playlist.stavke->len){
				playlist.trenutna = playlist.svira;
				cpu_ciscenje = 1;
			}
		} else if (ch == komande.tmuzika_first_song){
			playlist.trenutna = 0;
			horizontal_scroll = 0;
			cpu_ciscenje = 1;
		} else if (ch == komande.tmuzika_last_song){
			if (playlist.stavke->len > 0){
				playlist.trenutna = playlist.stavke->len - 1;
				horizontal_scroll = 0;
				cpu_ciscenje = 1;
			}
		} else if (ch == komande.tmuzika_repete_song){
			...
			cpu_ciscenje = 1;
		} else if (ch == komande.tmuzika_repete_all){
			...
			cpu_ciscenje = 1;
		} else if (ch == komande.tmuzika_shutlle){
			...
			cpu_ciscenje = 1;
		} else if (ch == komande.tmuzika_scroll_left){
			...
			cpu_ciscenje = 1;
		} else if (ch == komande.tmuzika_scroll_right){
