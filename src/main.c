...
void footer_lista(WINDOW *win){
	...
	const char *tekst; 
	if (radio_prozor){
		tekst = _(" ENTER/SPACE:play/pause ↑↓:move m:manager s:search ESC:quit ");
	} else{
		tekst = _(" ENTER/SPACE:play/pause ↑↓:move m:manager s:search k:exit ");
	}
...
void footer_opcije(WINDOW *win){
	...
	int fy = h - 1;
	mvwprintw(win, fy, 0, "%*s", w, "");
	const char *tekst = _(" ENTER:open h:reset key b:reset color ESC:quit ");
	wchar_t wtekst[256];
...
void footer_datoteka(WINDOW *win){
	...
	int fy = h - 1;
	mvwprintw(win, fy, 0, "%*s", w, "");
	const char *tekst = _(" ENTER:open/add BACKSPACE:back f:add folder o:options ?:help ESC:quit ");
	wchar_t wtekst[256];
	mbstowcs(wtekst, tekst, sizeof(wtekst)/sizeof(wchar_t));
	int len = wcswidth(wtekst, wcslen(wtekst));
	int start_linije = 2 + len;
...
static void fm_crtaj(WINDOW *win, FileManager *fm){
	...
	// Scroll za putanju
	int path_max_w = w - 4;
	if ((int)strlen(fm->trenutni_put) <= fm->x_offset){
		fm->x_offset = strlen(fm->trenutni_put) - 1;
	}
	if (fm->x_offset < 0){
		fm->x_offset = 0;
	}
	char *scrolled_path = fm->trenutni_put + fm->x_offset;
	int path_len = strlen(scrolled_path);
	if (path_len > path_max_w){
		path_len = path_max_w;
	}
	mvwprintw(win, 3, 2, "%.*s", path_len, scrolled_path);
	int list_start = 5;
	...
	for (int i = 0; i < max_prikaz; i++){
		...
		// horizontalni scroll samo za ime, prefiks uvek vidljiv
		int prefix_len = strlen(prefix);
		int col_start = 3;
		int avail_w = w - col_start - 1 - prefix_len;
		int name_len = strlen(ime);
		int offset = fm->x_offset;
		if (offset > name_len){
			offset = name_len;
		}
		int prikaz_len = name_len - offset;
		if (prikaz_len > avail_w){
			prikaz_len = avail_w;
		}
		char *display_name = ime + offset;
		mvwprintw(win, y, col_start, "%s%.*s", prefix, prikaz_len, display_name);
		// isključi boju
		if (!marked){
...
static void fm_otvori_config(const char *ime_fajla){
	...
	snprintf(putanja, sizeof(putanja), "%s/.tmuzika/config/%s", g_get_home_dir(), ime_fajla);
	endwin();
	pid_t pid = fork();
	if (pid == 0){
...
static int fm_key(FileManager *fm, int ch){
	...
	switch (ch){
		...
		case 14:{ // napravi novu fasciklu u popup-u
			...
			memset(ime_nove_fascikle, 0, sizeof(ime_nove_fascikle));
			int h, w;
			getmaxyx(stdscr, h, w);
			// Dimenzije popup-a
			int popup_h = 5;
			...
			wattron(popup, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
			box(popup, 0, 0);
			wattroff(popup, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
			const char *prompt = _(" Enter new folder name: ");
			mvwprintw(popup, 2, 2, "%s", prompt);
			wrefresh(popup);
			wchar_t wprompt[256];
			mbstowcs(wprompt, prompt, sizeof(wprompt)/sizeof(wchar_t));
			int prompt_width = wcswidth(wprompt, wcslen(wprompt));
			echo();
			curs_set(1);
			wmove(popup, 2, 2 + prompt_width);
			wattron(popup, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_OBAVESTENJE_PAIR));
			wgetnstr(popup, ime_nove_fascikle, sizeof(ime_nove_fascikle) - 1);
			wattroff(popup, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_OBAVESTENJE_PAIR));
			noecho();
			curs_set(0);
			werase(popup);
			wrefresh(popup);
		...
		case 20:{ // napravi novi dokument u popup-u
			char ime_nove_datoteke[256];
			memset(ime_nove_datoteke, 0, sizeof(ime_nove_datoteke));
			int h, w;
			getmaxyx(stdscr, h, w);
			int popup_h = 5;
			int popup_w = w / 2;
			...
			box(popup, 0, 0);
			wattroff(popup, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
			// Prompt unutar popup-a
			const char *prompt = _(" Enter new text file name: ");
			mvwprintw(popup, 2, 2, "%s", prompt);
			wrefresh(popup);
			// Prebaci prompt u wide string da merimo širinu u kolonama
			wchar_t wprompt[256];
			mbstowcs(wprompt, prompt, sizeof(wprompt)/sizeof(wchar_t));
			int prompt_width = wcswidth(wprompt, wcslen(wprompt));
			// Postavi echo i kursor
			echo();
			curs_set(1);
			// Postavi kursor odmah posle vizuelnog kraja prompta
			wmove(popup, 2, 2 + prompt_width);
			wattron(popup, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_OBAVESTENJE_PAIR));
			wgetnstr(popup, ime_nove_datoteke, sizeof(ime_nove_datoteke) - 1);
			wattroff(popup, COLOR_PAIR(TMUZIKA_PLAYLIST_FOOTER_OBAVESTENJE_PAIR));
			noecho();
			curs_set(0);
			// Zatvori popup
			...
			wrefresh(popup);
			delwin(popup);
			// Ako je uneseno ime, kreiraj fajl
		...
		case 's':{
			...
			while (1) {
				...
				if (ch == 27){ // ESC
					...
					i--;
					query[i] = '\0';
					mvwaddch(popup, 2, 2 + prompt_width + i, ' ');
					wmove(popup, 2, 2 + prompt_width + i);
				} else if (isprint(ch) && i < (int)sizeof(query) - 1){
					query[i++] = ch;
					mvwaddch(popup, 2, 2 + prompt_width + i - 1, ch);
				}
				wrefresh(popup);
			}
			noecho();
		...
		case 1:{ // ctrl+a Selektuj sve stavke
			...
			fm_cpu_ciscenje = TRUE;
			break;
		} case KEY_F(2): { // F2 Rename sa popup-om
			if (fm->stavke->len == 0){
				break;
			}
			...
			wattroff(popup, COLOR_PAIR(TMUZIKA_BORDER_PAIR));
			char prompt[512];
			snprintf(prompt, sizeof(prompt), _(" Rename \"%s\" to: "), old_name);
			mvwprintw(popup, 2, 2, "%s", prompt);
			wrefresh(popup);
			wchar_t wprompt[512];
			mbstowcs(wprompt, prompt, sizeof(wprompt)/sizeof(wchar_t));
			int prompt_width = wcswidth(wprompt, wcslen(wprompt));
			wattrset(popup, A_NORMAL);
			echo();
			curs_set(1);
			int ch, i = 0;
			int start_x = 2 + prompt_width;
			wmove(popup, 2, start_x);
			while (1){
				ch = wgetch(popup);
				if (ch == 27){ // ESC
					new_name[0] = '\0';
					...
					i--;
					new_name[i] = '\0';
					mvwaddch(popup, 2, start_x + i, ' ');
					wmove(popup, 2, start_x + i);
				} else if (isprint(ch) && i < (int)sizeof(new_name) - 1){
					new_name[i++] = ch;
					mvwaddch(popup, 2, start_x + i - 1, ch);
				}
				wrefresh(popup);
			}
			noecho();
		...
		case ']':{  // scroll levo
			fm->x_offset -= 5;
			if (fm->x_offset < 0) fm->x_offset = 0;
			break;
		}
		case 'o':{
...
void radio_prikaz(WINDOW *win){
	...
	for (int i = 0; i < show && (start + i) < (int)radio.stanice->len; i++){
		...
		if (svira_ova && radio.online){
			...
			wattron(win, COLOR_PAIR(TMUZIKA_PLAYLIST_TEKST_PAIR) | A_REVERSE);
		} else {
		...
		if (svira_ova && radio.online){
			...
			wattroff(win, COLOR_PAIR(TMUZIKA_PLAYLIST_TEKST_PAIR) | A_REVERSE);
		} else {
...
void glava(WINDOW *win){
	...
	if (radio_prozor){
		...
		mvwprintw(win, 4, 2, _("? = help  o = config"));
...
static void pomoc_komande(WINDOW *win){
	...
	snprintf(linije_tmuzika[t++],128,"%s = %s",K(komande.tmuzika_current_song),_("go to the current song"));
	...
	snprintf(linije_radio[r++],128,"%s = %s",K(komande.tradio_current_song),_("go to the current radio station"));
	...
	snprintf(linije_radio[r++], 128, "ESC = %s", _("quit"));
